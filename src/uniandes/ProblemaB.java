...
public class ProblemaB {
	...
	public static void main(String[] args) throws Exception {
		...
		try (
			...
			String line;
			while (
					(line = br.readLine()) != null && line.length() > 0 && !"0".equals(line)
					) {
				int rodSize = Integer.parseInt(line);
		...
		for (Tupla<Integer, int[]> problema : problemas) {
			int solucion = instancia.costoMinimoCortes(
					problema.left(),
					problema.right()
					);
	...
	public int costoMinimoCortes(Integer tamano, int[] puntosCorte) {
		...
		for (int i = 0, j = i, z = 0; z < t.length;) {
			...
			if (i >= j) {
				//no puedo empezar adelante del final y no puedo cortar si empiezo y acabo en el mismo punto
				t[i][j] = 0;
			} else {
				t[i][j] = -1;
	...
	public int minPositive(int a, int b) {
		if (a < 0) return b; else if (b < 0) return a; else if (b < a) return b;
		return a;
	}
}
